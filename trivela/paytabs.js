...
async function createPaymentPage(order, returnUrl, callbackUrl) {
  ...
  const payload = {
    ...
    tran_class: "ecom",
    cart_id: order.id,
    cart_description: order.service || "طلب من متجر ShopCoin",
    cart_currency: currency,
    cart_amount: cartAmount,
    callback: callbackUrl,
    return: returnUrl,
    customer_details: {
      name: order.customerName || order.user_name || "عميل شوب كوينز",
      email: order.customerEmail || order.user_email || order.eaEmail || "customer@shopcoin.local",
